...
The spectrum displays two noteworthy features: a large spike near 0, and a bump near 1. The bump corresponds to turbines working near full capacity. The large spike correlates with low wind velocities. Part of it comes from turbine outputs that are exactly zero; these are shown by orange dots in Figure 2. The fraction of turbine outputs that are zero is 8.5% for all turbines combined, but varies from 2.4% for turbine 2 to 17.0% for turbine 9. Figure 2 illustrates that the wind speed must reach a minimum value in order for a turbine to start producing power. This minimum is known as the *cut-in speed*{% sidenote 'WTSN-2' 'For some basic explanations of the workings of wind turbines, see the [Windpower Program](http://www.wind-power-program.com/turbine_characteristics.htm) website.' %}.

{% maincolumn "assets/img/blog/WindTurbines/donut_plots.png" "Figure 2: Scatter plots of zonal versus meridional wind velocity components at heights of 10m (left) and 100m (right) above ground, for turbine 9. Turbine output is zero for orange points and greater than zero for blue points. The orange points cluster around low wind speeds, whereas the blue points form a donut shape around the orange ones." %}
